...
plt.show()
plt.savefig('graph/keras13_diabets')

# 그림 도표를 보면서 과적합이 언제 생기는 지 생각해보자
# epoch이 많을 수록 좋은 것이 아님 그래프를 보면 값들이 줄어들었다가 팡 튀고를 반복
# 계속 여러번 돌려보면서 loss와 var_loss 격차가 많이 줄어가는걸 보면서 epoch량을 조절한다.
# val_loss가 최저점이다라는 말의 뜻은 y = wx + b 예측을 가장 잘했다. 
# 편하게 최저점을 구하는 방법 
# 무한루프 돌리고 조건식을 설정해준다. 일정 최저값을 찍고 특정횟수의 유예를 준다. 그 유예동안 최저값 갱신이 안되면 다시 끊고 갱신이 되면 다시 루프돌린다.